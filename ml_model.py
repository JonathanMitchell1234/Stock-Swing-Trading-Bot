"""
ML Model wrapper – loads a trained LightGBM model and provides
predictions for the trading bot.
...
from ml_features import FEATURE_NAMES, NUM_FEATURES, extract_row
...
def _ensure_loaded() -> bool:
    ...
    with _lock:
        ...
        if not MODEL_PATH.exists():
            log.info("No GBM model found at %s — ML scoring disabled", MODEL_PATH)
            _loaded = True
            return False
        ...
        try:
            ...
            log.info("Loaded GBM model from %s (%d trees)",
                     MODEL_PATH, _booster.num_trees())
        except Exception as exc:
            log.warning("Failed to load GBM model: %s", exc)
            _booster = None
...
def predict_entry_proba(
    ...
    """
    Predict the probability that entering at bar *idx* leads to a
    profitable trade.
    ...
    fv = extract_row(df, idx=idx, weekly_bullish=weekly_bullish,
    ...
    try:
        ...
        log.warning("GBM prediction failed: %s", exc)
        return None
...
def predict_entry_batch(
    ...
    try:
        ...
        log.warning("GBM batch prediction failed: %s", exc)
        return None
