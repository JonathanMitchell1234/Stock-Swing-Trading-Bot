"""
ML Trainer for SHORT entries – builds a LightGBM gradient-boosting model
for short-sell entry prediction.

Usage (CLI):
    python ml_trainer_short.py                          # train on all watchlist symbols, 2 years
    python ml_trainer_short.py --symbols AAPL MSFT NVDA # specific symbols
    python ml_trainer_short.py --months 36              # 3 years of data
    python ml_trainer_short.py --tune                   # run Optuna hyper-param search

The trained model is saved to  models/gbm_short.txt
and a JSON metrics file to    models/gbm_short_meta.json

This is structurally identical to ml_trainer.py but:
  - Uses short-specific feature extraction (ml_features_short)
  - Uses short-specific labels (price DROP instead of gain)
  - Saves to separate model files (gbm_short.txt)
"""
...
from ml_features_short import (
    build_short_feature_matrix,
...
log = get_logger("ml_trainer_short")
...
# ═════════════════════════════════════════════════════════════
# Dataset construction (short-specific)
# ═════════════════════════════════════════════════════════════
...
def build_short_dataset(
    ...
    """
    Build (X, y, weights, symbols_per_row) for SHORT training.
    Labels are inverted: label=1 when price drops by min_drop_pct.
    """
    ...
    for i, (sym, df) in enumerate(data.items()):
        ...
        X, valid_idx = build_short_feature_matrix(
        ...
        log.info("  %s: %d samples (%.1f%% positive/short-profitable)",
...
def save_short_model(bst, meta: dict) -> None:
    MODEL_DIR.mkdir(exist_ok=True)
    bst.save_model(str(MODEL_PATH))
    META_PATH.write_text(json.dumps(meta, indent=2))
    log.info("Short model saved to %s", MODEL_PATH)
    log.info("Short metadata saved to %s", META_PATH)


def load_short_meta() -> Optional[dict]:
    if META_PATH.exists():
...
def run_short_training(
    ...
    """
    Full SHORT training pipeline. Returns the metadata dict.
    Can be called programmatically (from dashboard) or from CLI.
    """
    symbols = symbols or list(config.WATCHLIST)
    # Remove symbols from SHORT_BLACKLIST — can't short them
    blacklist = set(getattr(config, "SHORT_BLACKLIST", []))
    ...
    if not data:
        raise RuntimeError("No data loaded — cannot train short model")
    ...
    # 2. Build dataset
    log.info("Step 2/3: Extracting SHORT features & labels…")
    X, y, weights, sym_labels = build_short_dataset(
        data,
        forward_bars=forward_bars,
        min_drop_pct=min_drop_pct,
    ...
    log.info("Short dataset: %d samples, %d features, %.1f%% positive (drop occurred)",
    ...
    # 3. Train
    log.info("Step 3/3: Training SHORT LightGBM…")
    best_params = None
    if tune:
    ...
    meta["cv_threshold"] = getattr(config, "ML_SHORT_ENTRY_THRESHOLD",
                                    config.ML_ENTRY_THRESHOLD)
    meta["training_time_s"] = round(time.time() - t0, 1)

    save_short_model(bst, meta)

    # Print summary
    avg = meta["avg_metrics"]
    log.info("=" * 60)
    log.info("SHORT TRAINING COMPLETE in %.1fs", meta["training_time_s"])
...
def main():
    parser = argparse.ArgumentParser(description="Train the SHORT GBM entry model")
    parser.add_argument("--symbols", nargs="+", default=None,
                        help="Symbols to train on (default: full watchlist minus SHORT_BLACKLIST)")
    ...
    parser.add_argument("--min-drop-pct", type=float, default=0.03,
                        help="Minimum drop %% for positive label (default: 0.03 = 3%%)")
